...
----
## HTML 속성
* `<html lang="ko">` 라고 해더 위에 적으면 스크린리더기가 다른 언어보다 빠르게 읽는 언어를 설정할 수 있다.
## meta의 속성
* `<meta charset="UTF-8">`을 해더에 적으면 다국어를 인식할 수 있고 글이 깨지지 않는다.
* `<meta name="description" content="?">` 에서 물음표에 적는 말이 홈페이지 사이트 아래 설명으로 나오는 것이다.
* `<meta name="keywords" content="?">` 에서 물음표에 적는 말이 검색창에 검색을 했을 때 키워드로 나의 사이트를 잘 찾을 수 있게 설정하는 것이다.
* 위에 글에서 말한 키워드의 갯수 제한은 없다.